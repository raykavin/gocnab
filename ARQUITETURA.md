...
- **Preencher um campo** (`field.go`): campos numéricos (`9`) são alinhados à direita e completados com zeros à esquerda; um valor que não cabe no tamanho do campo é erro (nunca trunca dinheiro). Campos alfanuméricos (`X`) são maiúsculizados, alinhados à esquerda e completados com espaços à direita; um valor mais longo que o campo é truncado (comportamento real de bancos para nomes longos). Um `FieldSpec` pode marcar `Lowercase` para que um campo alfanumérico específico saia em minúsculas, exigência de alguns manuais para uma chave PIX de e-mail; a validação de caracteres continua rodando sobre a forma maiúscula, então a opção não amplia o conjunto permitido. Campos de documento (`D`, `KindDocument`) renderizam como numéricos, mas também aceitam um valor que já preencha a largura inteira do campo sem ser todo numérico, que é o que deixa um CNPJ alfanumérico da Receita Federal passar sem ser recusado como "não numérico". Casas decimais implícitas (notação `9(13)V2`) não multiplicam nada em tempo de execução: valores monetários chegam da camada 3 como inteiros no `Cents` (já na unidade mínima da moeda), então o preenchimento é só zero-padding; para outros campos numéricos com decimais, uma string como `"12.5"` é escalonada com aritmética inteira sobre a própria string, nunca com `float64`.
- **Validar o conjunto de caracteres** (`charset.go`): dígitos, letras maiúsculas, espaço e um conjunto de símbolos (`.,/-:()+*@_`, incluindo `@` e `_` porque sem eles uma chave PIX por e-mail nunca passaria). Um caractere fora desse conjunto (por exemplo, uma letra acentuada) é erro, não é descartado silenciosamente: renderizar um campo nunca sanitiza por conta própria, para que um caractere inválido sempre apareça como erro em vez de sumir do arquivo. A função `Sanitize` do motor, exposta na camada pública como `cnab.Sanitize`, troca acentos pelo equivalente sem acento e descarta o que continuar inválido; `cnab.AllowedInField` responde a mesma pergunta sem reescrever nada, para validar cedo um identificador que não pode ser alterado.
...
O registro por nome é single-shot e vale pelo resto da vida do processo, o que serve bem a um layout que é constante do binário e serve mal a um layout que é dado: um descritor carregado de um banco de dados, por exemplo, ou um layout por cliente resolvido a cada requisição. Para esses casos, `Config.LayoutSpec` recebe a instância de `Layout` diretamente e ignora o registro (`ParseReturnWithLayout` faz o mesmo na direção do retorno). É a mesma interface nos dois caminhos; muda só quem guarda a referência.
...
- **Chaves semânticas** (escritas pela camada `cnab`, lidas pelos `FieldSpec.Key` de cada layout): `KeyAmount`, `KeyPaymentDate`, `KeyPayeeName`, `KeyPixKeyValue`, `KeyBarcode`, `KeyTaxCode`, e assim por diante (lista completa em `cnab/layout/value.go` e em `API.md`).
...
`SegmentZ` é o único `RecordKey` do vocabulário que existe só na direção do retorno: nenhum tipo de pagamento o produz em uma remessa. Um banco o anexa depois do segmento principal de um movimento para informar a autenticação daquele movimento. O layout de referência `febraban240` não o implementa, porque o padrão FEBRABAN puro não fixa o conteúdo dele; um layout de banco que precise dessa informação declara o `RecordSpec` correspondente e `ParseReturn` passa a preencher `ReturnMovement.Authentication`/`BankControl`.
...
`File` e `Batch` orquestram a montagem: `NewRemittance` valida a configuração e resolve o layout, pelo nome em `Config.Layout` ou pela instância em `Config.LayoutSpec`; `NewBatch` cria um lote associado a um `BatchProduct`/`BatchService`; `AddPayment` valida o pagamento, converte em segmentos e confere que o layout ativo realmente suporta os segmentos necessários (rejeitando na hora um tipo de pagamento incompatível com o banco escolhido, em vez de só falhar depois em `Generate`); `Generate` monta o `engine.FileInput` completo e delega ao motor.

`ConvertToBarcode` é a única função desta camada que não participa da montagem do arquivo: ela recebe a linha digitável de um boleto (47 dígitos) ou de uma conta/tributo (48 dígitos), confere os dígitos verificadores FEBRABAN, devolve o código de barras de 44 dígitos que `BoletoPayment`/`BarcodeTax` esperam e diz a qual dos dois segmentos ele pertence. Fica aqui, e não no motor, porque a regra é do documento brasileiro sendo pago, não do formato de 240 colunas.
...
| Caracteres inválidos | Validado na renderização de cada campo alfanumérico, dentro de `Generate`; passe texto livre por `cnab.Sanitize` antes de montar o pagamento se a entrada pode conter acentos |
...
3. Ignora header/trailer de arquivo e de lote, e todo segmento de detalhe que não seja um **segmento principal de movimento**: o Segmento A (crédito em conta, TED, PIX), o Segmento J (boleto) ou o Segmento O (conta/tributo com código de barras).
4. Decodifica cada segmento principal com `engine.Engine.ParseRecord`, extraindo `KeyYourNumber`, `KeyAmount` e as chaves que só um arquivo de retorno preenche: `KeySettlementDate`, `KeySettlementAmount` e `KeyOccurrenceCodes` ("data/valor real da efetivação do pagamento" e "códigos/motivos de ocorrências para retorno").
5. Quando um Segmento Z vem logo depois de um segmento principal, anexa `KeyAuthentication`/`KeyBankControl` àquele movimento em vez de abrir um novo.

Um Segmento J cujo campo "Código Reg. Opcional" (colunas 18-19) contém `"52"` é um J-52, registro de continuação do Segmento J anterior, e é pulado como qualquer outro segmento complementar.

Por que só o segmento principal: ele já carrega tudo que uma reconciliação de pagamento precisa (liquidou? quando? por quanto? se não liquidou, por quê? qual é o "seu número" para casar de volta com o pagamento original). Os Segmentos B/BPix/J-52 carregam documento, endereço e chave PIX do favorecido, dados que quem chama já tem nos seus próprios registros, e que, no caso de B vs. BPix, exigiriam olhar o Segmento A anterior (ou o conteúdo físico do próprio B) para desambiguar qual dos dois layouts uma linha "B" representa, já que o padrão FEBRABAN reaproveita a mesma letra para os dois. O Segmento N (DARF/GPS) também não é decodificado: um layout de banco que precise dele se estende do mesmo jeito.

As chaves de retorno do Segmento A foram acrescentadas a `layouts/febraban240/segment_a.go` reaproveitando colunas que já existiam como `Const "0"`/preenchimento em branco (o padrão sempre reservou essas colunas ali; só não havia leitor para elas). Isso é compatível com qualquer código de geração existente: uma chave de `Values` não definida renderiza exatamente igual ao `Const` que ela substituiu (ver `resolveValue` em `field.go`), então nenhum tipo de pagamento precisou mudar.
...
Cada camada tem sua própria suíte de testes, todas rodando com `go test ./... -cover`:

- `internal/engine` cobre preenchimento, alinhamento e truncamento, leitura simétrica de campo e registro (incluindo o caso de divergência em campo `Const`), classificação de linha, montagem completa de arquivo com verificação byte a byte (incluindo o CRLF), cálculo de trailers e sequenciais com múltiplos lotes, e os casos de limite excedido, mantendo cobertura acima de 85%.
- `cnab/layout` cobre o registro (duplicidade, concorrência), a validação de `RecordSpec` e o carregador de JSON.
- `layouts/febraban240` confere que cada registro cobre exatamente as 240 colunas e que o motor aceita o layout inteiro.
- `cnab` cobre a validação de cada tipo de pagamento, a conversão de linha digitável em código de barras com seus dígitos verificadores, o fluxo ponta a ponta contra `febraban240`, `ParseReturn` contra uma remessa gerada pelo próprio SDK com as colunas de retorno preenchidas manualmente (não há amostra de um banco real para um layout de referência que não representa banco nenhum), e a tradução de erro na fronteira pública tanto para `ParseReturn` quanto para `Generate` (este via um número de conta de favorecido maior do que a coluna do layout suporta, que `AddPayment` não tem como recusar antes de o motor tentar renderizá-lo).