...
- **Validar o conjunto de caracteres** (`charset.go`): dígitos, letras maiúsculas, espaço e um conjunto de símbolos (`.,/-:()+*@_`, incluindo `@` e `_` porque sem eles uma chave PIX por e-mail nunca passaria). Um caractere fora desse conjunto (por exemplo, uma letra acentuada) é erro, não é descartado silenciosamente: renderizar um campo nunca sanitiza por conta própria, para que um caractere inválido sempre apareça como erro em vez de sumir do arquivo. O motor tem uma função `Sanitize` que troca acentos pelo equivalente sem acento e descarta o que continuar inválido, mas ela é interna ao módulo; quem usa o SDK de fora precisa normalizar nomes e textos livres antes de montar o pagamento.
...
| Caracteres inválidos | Validado na renderização de cada campo alfanumérico, dentro de `Generate`; normalize acentos nos dados de entrada antes de montar o pagamento |
